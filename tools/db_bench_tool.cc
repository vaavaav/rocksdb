...
using GFLAGS_NAMESPACE::SetUsageMessage;
    ...
    DEFINE_string(
        ...
        "Meta operations:\n"
        "\tcompact     -- Compact the entire DB; If multiple, randomly choose "
        "one\n"
        ...
        "\tgetmergeoperands -- Insert lots of merge records which are a list "
        "of "
        "sorted ints for a key and then compare performance of lookup for "
        "another "
        "key "
        "by doing a Get followed by binary searching in the large sorted list "
        "vs "
        "doing a GetMergeOperands and binary searching in the operands which "
        "are"
...
// optimization, so skip the check of the max level.
  ...
  // scattered around the keyspace if they are keyed by userid.timestamp. A
  // workload which orders items purely by time, and demands the latest, is very
  // different than workload here (which we believe is more typical of how
